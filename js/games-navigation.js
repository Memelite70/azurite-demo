...
localStorage.setItem("recentGames", JSON.stringify(t))
}
...
function loadGameOptions(e) {
  ...
  n="all"===a ? [...gamesList] : "new"===a ? gamesList.slice(-20) : "recent"===a ? JSON.parse(localStorage.getItem("recentGames")) || [] : gamesList.filter((e=> Array.isArray(e.Categories) && e.Categories.some((e=> e.toLowerCase()===a)))),
  n.sort(((e, t)=> e.title.toLowerCase().localeCompare(t.title.toLowerCase()))),
  n.forEach((e, i)=> {
          const gameDiv=document.createElement("div");
          gameDiv.className="gameframe_game";
          gameDiv.setAttribute("onclick", `opengame('${e.url}'); `);
          const img=document.createElement("img");
          img.setAttribute("onclick", "localStorage.setItem('gameimage', this.src);");
          img.setAttribute("loading", "lazy");
          img.src=e.image || "./globe.svg";
          img.alt=`${e.title} unblocked`;
...
const n=gamesList .filter(t=> t.title.toLowerCase().includes(e.toLowerCase()) || (Array.isArray(t.Categories) && t.Categories.some(c=> c.toLowerCase().includes(e.toLowerCase())))) .sort((a, b)=> a.title.localeCompare(b.title));

if (n.length===0) {
  gamesMain.innerHTML="<p style='padding:8px;color:#aaa;'>No results found.</p>";
  gamesMain.style.display="block";
  return;
}
...
n.forEach(game=> {
      const gameDiv=document.createElement("div");
      gameDiv.className="gameframe_game";
      gameDiv.setAttribute("onclick", `opengame('${game.url}'); `);

      const img=document.createElement("img");
      img.setAttribute("onclick", "localStorage.setItem('gameimage', this.src);");
      img.setAttribute("loading", "lazy");
      img.src=game.image || "./globe.svg";

      img.alt=`${game.title} unblocked`;
